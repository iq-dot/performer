...
{	
	...
	listDc->startSection("push_back"); //measure list push_back
	...
	vecDc->startSection("push_back");
	for(int i=0; i<=100; i++){
		intVec.push_back(i);
	}
	vecDc->stopSection();
	
	listDc->startSection("push_back");
	for(int i=0; i<100; i++){
		intList.push_back(i);
	}
	listDc->stopSection();
	...
	//get the speed and memory results
	speed_map sm;
	mem_map mm;
	
	vecDc->getSpeedResult(&sm);
	vecDc->getMemResult(&mm);
	
	//analyze and print analyzed result
	Analyzer *anz = pf->createAnalyzer(vecConf, sm);
	cout << anz->getResult() << endl;
